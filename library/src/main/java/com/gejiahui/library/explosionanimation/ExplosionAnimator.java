...
public class ExplosionAnimator extends ValueAnimator {
    ...
    private final Interpolator DEFAULT_INTERPOLATOR = new AccelerateInterpolator(0.6f);
    private final float END_VALUE = 1.4f;
    ...
    public ExplosionAnimator(View container, Bitmap bitmap, Rect bound, Particle particle,Constant.Mode mode) {
    ...
    public void  generateParticles(Bitmap bitmap ,Random random,Particle particle,Constant.Mode mode){
        int w = 0,h = 0,parts = 0;
        if(mode == Constant.Mode.view){
            parts = partLen;
        }
        else{
